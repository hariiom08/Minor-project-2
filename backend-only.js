/**
 * Backend-only Server for Quiz Application
 * 
 * This script provides a simplified backend API server without using any routing 
 * libraries that might cause path-to-regexp errors.
 */
...
const server = http.createServer(async (req, res) => {
  ...
  if (path.match(/^\/api\/quizzes\/[^\/]+\/submit$/) && req.method === 'POST') {
    ...
    // Process user answers with details
    const answersWithDetails = data.answers.map(answer => {
      const question = questions.find(q => q.id === answer.questionId);
      if (!question) return null;
      
      return {
        questionId: answer.questionId,
        questionText: question.text || `Question ${answer.questionId}`,
    ...
    // Create result with detailed answers
    const result = {
      id: (userResults.length + 1).toString(),
      userId: decoded.id,
      quizId,
      quizTitle: quizzes.find(q => q.id === quizId)?.title || `Quiz ${quizId}`,
      score,
  ...
  if (path === '/api/quiz-history' && req.method === 'GET') {
    // Get token from headers
    const token = extractToken(req.headers.authorization);
    ...
    const userQuizResults = userResults.filter(r => r.userId === decoded.id);
    
    // Sort by date (most recent first)
    userQuizResults.sort((a, b) => new Date(b.completedAt) - new Date(a.completedAt));
    
    // Get limit from query params (default to 5)
    const limit = parseInt(parsedUrl.query.limit) || 5;
    
    // Limit the results
    const limitedResults = userQuizResults.slice(0, limit);
    
    // Attach quiz info to each result
    const resultsWithQuizInfo = limitedResults.map(result => {
  ...
  if (path === '/' || path === '') {
    ...
    res.end(`
      ...
      <html>
        <head>
          <title>Quiz Application Backend Server</title>
          <style>
            body {
        ...
        <body>
          <h1>Quiz Application Backend Server</h1>
          
          <div class="alert success">
            ✅ Backend server is running at http://${HOST === '0.0.0.0' ? 'localhost' : HOST}:${PORT}
          </div>
          
          <div class="alert warning">
            <strong>Note:</strong> This is a backend-only development server.
            <br>For client-side development, start the React frontend separately.
          </div>
          ...
          <ul>
            ...
            <li><code>GET /api/quiz-history</code> - Get recent quiz history (requires auth)</li>
...
server.listen(PORT, HOST, () => {
  console.log(`
========================================
🚀 Quiz Application Backend Server
========================================
Server is running at: http://${HOST === '0.0.0.0' ? 'localhost' : HOST}:${PORT}
API available at: http://${HOST === '0.0.0.0' ? 'localhost' : HOST}:${PORT}/api
Health check: http://${HOST === '0.0.0.0' ? 'localhost' : HOST}:${PORT}/api/health
----------------------------------------
